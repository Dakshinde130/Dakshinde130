<!--
  Profile README  →  repo name must be: Dakshinde130
  Warm "night ramen" theme. Zero assets. Paste once, works everywhere.
-->

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=soft&color=0:08080f,45:1a0e08,100:08080f&height=300&section=header&text=Daksh%20Shinde&fontSize=84&fontColor=f3ead9&fontAlignY=40&desc=Final%20year%20Computer%20Science%20student%20who%20likes%20building%20with%20AI&descAlignY=60&descSize=15&descColor=ffb56b&animation=fadeIn" width="100%"/>

</div>

<br/>

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Sora&weight=400&size=23&duration=4000&pause=2000&color=FFB56B&center=true&vCenter=true&width=720&lines=Final+year+Computer+Science+student;I+build+things+with+AI;I+also+love+finance+and+trading;Looking+for+a+job+or+internship)](https://git.io/typing-svg)

</div>

<br/>

<div align="center">

![](https://img.shields.io/badge/Computer%20Science%20Student-1a0e08?style=flat-square&logoColor=ffb56b)&nbsp;
![](https://img.shields.io/badge/SGGS%20Institute-1a0e08?style=flat-square)&nbsp;
![](https://img.shields.io/badge/Maharashtra%2C%20India-1a0e08?style=flat-square&logo=googlemaps&logoColor=ffb56b)&nbsp;
![](https://img.shields.io/badge/Open%20to%20Work-1a0e08?style=flat-square&logo=checkmarx&logoColor=3ecf8e)

<br/><br/>

### [**↗ &nbsp;See my portfolio website**](https://dakshinde130.github.io)

</div>

<br/><br/>

---

<br/>

## &nbsp;Hi, I'm Daksh 👋

I'm a final year Computer Science student at SGGS Institute of Engineering and Technology, with a minor in Data Science.

I like building things. Mostly with AI, but I'm also really into finance and trading. Most of my projects started as class projects or just ideas I wanted to try, and I built them to actually work, not just to look good in a demo.

Right now I'm looking for a job or internship where I can keep learning and build real things with a team.

<br/>

**Quick facts:**

| | |
|---|---|
| 🎓 &nbsp;Studying | Computer Science + Data Science minor |
| 🏫 &nbsp;College | SGGS Institute of Engineering and Technology |
| 📍 &nbsp;From | Maharashtra, India |
| 💼 &nbsp;Status | Open to jobs and internships |
| 🗣️ &nbsp;Speaks | English, Hindi, Marathi, a little Japanese |

<br/><br/>

---

<br/>

## &nbsp;Things I've Built

Here are four projects I'm proud of. Click any of them to see the code.

<br/>

### 🎯 &nbsp;[Anikator](https://github.com/Dakshinde130/anikator)
A guessing game like Akinator. You think of an anime character, and it asks you questions until it figures out who you picked. It uses probability to guess smart, not random. I built the whole thing on my own.

`Python` &nbsp; `Bayesian Logic` &nbsp; `Decision Trees` &nbsp; `React`

<br/>

### 👗 &nbsp;[SmartStyle AI](https://github.com/Dakshinde130/SmartStyle-AI-based-Family-Wardrobe-and-Outfit-Recommendation-System)
An app that helps you manage your wardrobe and suggests outfits to wear. It uses computer vision to look at your clothes, has secure OTP login, and saves everything to the cloud. It works for one person, a couple, or a whole family.

`Python` &nbsp; `Computer Vision` &nbsp; `React` &nbsp; `Supabase`

<br/>

### 🔄 &nbsp;[SkillSwap](https://github.com/Dakshinde130/skillswap)
A simple website where students teach each other. You share a skill you know, and you learn a skill from someone else. No money, just trading what you're good at. I made it because I think classmates can learn a lot from each other.

`Python` &nbsp; `Streamlit` &nbsp; `Pandas` &nbsp; `JSON`

<br/>

### 🍳 &nbsp;[Recipe Generator](https://github.com/Dakshinde130/recipe-generator)
A Java app that gives you recipe ideas. You tell it what you like or what you have, and it pulls recipes from an online API. I focused on keeping the code clean and easy to add to later.

`Java` &nbsp; `APIs` &nbsp; `Object-Oriented Programming`

<br/><br/>

---

<br/>

## &nbsp;What I Know

<div align="center">

<img src="https://skillicons.dev/icons?i=python,java,js,c,react,supabase,git,github,vscode&theme=dark&perline=9"/>

</div>

<br/>

| Area | What I use |
|------|-----------|
| 🧠 &nbsp;**AI & Data** | Machine Learning, Computer Vision, Decision Trees, Bayesian Logic |
| 💻 &nbsp;**Languages** | Python, Java, JavaScript, C, SQL |
| 🌐 &nbsp;**Web & Apps** | React, Streamlit, APIs, Supabase |
| 📊 &nbsp;**Finance** | Trading basics, financial analysis, working with data |
| 🔧 &nbsp;**Tools** | Git, GitHub, VS Code |

<br/><br/>

---

<br/>

## &nbsp;Courses I've Finished

I like learning outside of class too. Here are some courses I've completed:

- **Oracle Cloud AI Foundations + Generative AI Professional** — Oracle (2025)
- **Trading Strategies in Emerging Markets** — Indian School of Business
- **Machine Learning for All** — University of London
- **Python and Statistics for Financial Analysis** — HKUST
- **Data Structures and Algorithms** — UC San Diego
- **Using Python to Access Web Data** — University of Michigan
- **Claude 101 and AI Fluency** — Anthropic
- Took part in the **Smart India Hackathon**

<br/><br/>

---

<br/>

## &nbsp;Leadership

I've helped run **Pragyaa**, my college's big yearly tech and cultural fest, for three years. Each year I got more responsibility:

- **2023–24** — Started as a Hospitality Trainee
- **2024–25** — Became Joint Coordinator for Public Relations
- **2025–26** — Now I'm the Hospitality and Guest Management Lead

I led teams, handled guests, and worked with lots of different groups to pull off big events. I learned a lot about staying calm and getting things done when things get busy.

<br/><br/>

---

<br/>

## &nbsp;My GitHub Stats

## My GitHub Stats

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=Dakshinde130&theme=gruvbox&no-frame=true&no-bg=true&column=7&margin-w=8&margin-h=8"/>

<br/><br/>

<img height="175em" src="https://github-readme-stats.vercel.app/api?username=Dakshinde130&show_icons=true&include_all_commits=true&count_private=true&hide_border=true&cache_seconds=1800&bg_color=08080f&title_color=ffb56b&icon_color=ff7a3c&text_color=cdbfa8&ring_color=ff7a3c"/>
&nbsp;
<img height="175em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Dakshinde130&layout=compact&langs_count=6&hide_border=true&cache_seconds=1800&bg_color=08080f&title_color=ffb56b&text_color=cdbfa8"/>

<br/><br/>

<img width="92%" src="https://github-readme-activity-graph.vercel.app/graph?username=Dakshinde130&bg_color=08080f&color=ffb56b&line=ff7a3c&point=f3ead9&area=true&hide_border=true&custom_title=My%20Contribution%20Graph"/>

</div>

## &nbsp;Say Hi

I'm looking for a job or internship. If you think I'd be a good fit for something, please reach out. I'm happy to chat.

<br/>

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-1a0e08?style=for-the-badge&logo=linkedin&logoColor=ffb56b)](https://linkedin.com/in/daksh-shinde)&nbsp;&nbsp;
[![GitHub](https://img.shields.io/badge/GitHub-1a0e08?style=for-the-badge&logo=github&logoColor=ffb56b)](https://github.com/Dakshinde130)&nbsp;&nbsp;
[![Email](https://img.shields.io/badge/Email-1a0e08?style=for-the-badge&logo=gmail&logoColor=ffb56b)](mailto:dakshshinde007@email.com)

<br/>

![](https://komarev.com/ghpvc/?username=Dakshinde130&style=flat-square&color=9a8a72&label=profile+views)

<br/>

<sub>Thanks for stopping by. Still learning, still building. 🍜</sub>

</div>
